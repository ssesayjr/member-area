...
var cacheName = 'sw-precache-v3-sw-precache-' + (this.registration ? this.registration.scope : '');
...
var urlsToCacheKeys = new Map(
  precacheConfig.map(function (item) {
    ...
    var hash = item[1];
    var absoluteUrl = new URL(relativeUrl, window.location.pathname);
...
this.addEventListener('fetch', function (event) {
  if (event.request.method === 'GET') {
    ...
    if (!shouldRespond &&
      ...
      url = new URL(navigateFallback, window.location.pathname).toString();
      shouldRespond = urlsToCacheKeys.has(url);
    }
